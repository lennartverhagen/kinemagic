...
while j < length(t)
    ...
    %----------------------------------------
    % get forces
    iFs(j,:)	= A .* Fs_fun(ip,igoal,rg*3);           	% goal attractor
    ...
    iFd(j,:)	= D .* Fd_fun(ip,iv,igoal,tp,tgoal,rg*3);	% damping and simultaneous arrival
    
    % combine all forces acting on the digit
    iF(j,:) = iFs(j,:) + iFrt(j,:) + iFro(j,:) + iFb(j,:) + iFd(j,:);
    ...
    %----------------------------------------
    % get forces
    tFs(j,:)	= A .* Fs_fun(tp,tgoal,rg*3);
    tFrt(j,:)   = Rt.* Frt_fun(tp,tv,ttarget,tbt);
    tFro(j,:)   = Ro.* Fro_fun(tp,tv,obstacle,bo);
    tFb(j,:)	= K .* Fb_fun(tp,ip);
    tFd(j,:)	= D .* Fd_fun(tp,tv,tgoal,ip,igoal,rg*3);
    
    % combine all forces acting on the digit
    tF(j,:) = tFs(j,:) + tFrt(j,:) + tFro(j,:) + tFb(j,:) + tFd(j,:);
...
%     end
    ...
    % stop when the target is almost reached to avoid x/0
    if euclid(igoal-ipos(j+1,:)) < rg || ...
       euclid(tgoal-tpos(j+1,:)) < rg
        ivel(j+1,:) = [NaN NaN];
        tvel(j+1,:) = [NaN NaN];
        break
    end
...
function Fs = Fs_fun(p,goal,rg)
%% move digits to goal positions
%----------------------------------------
% Fs = A./dg
%
% Fs: attracting force
